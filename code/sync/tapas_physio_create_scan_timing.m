    ...
    case 'scan_timing_log'
        switch lower(log_files.vendor)
            case 'siemens'
                ...
                if durationPhyslogAfterEndOfLastScan < 0
                    N_pad_samples = ceil(abs(durationPhyslogAfterEndOfLastScan) / ons_secs.dt);
                    ons_secs.t = [ons_secs.t; ons_secs.t(end)+(1:N_pad_samples)'*ons_secs.dt];
                    ons_secs.c = [ons_secs.c; zeros(N_pad_samples,1)];
                    ons_secs.r = [ons_secs.r; zeros(N_pad_samples,1)];
                    
                verbose = tapas_physio_log(...
                    sprintf('padded %d samples: logfile too short', ...
                    N_pad_samples), verbose, 1);
